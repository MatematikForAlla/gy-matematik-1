% $Id$
%
% Copyright 2011, 2012, 2013, 2014; Daniel Bosk <daniel@bosk.se>
%
% This work is licensed under the Creative Commons Attribution-ShareAlike 3.0 
% Unported license.  To view a copy of this license, visit URL
%
%   http://creativecommons.org/licenses/by-sa/3.0/.
%

\documentclass[a4paper,titlepage,reqno,final,oneside]{amsbook}
%\documentclass[a4paper,titlepage,reqno,final,twoside]{amsbook}
%\documentclass[a4paper,reqno,twoside,draft]{amsbook}
\usepackage{amsbooka} % for chapter-specific authors

\usepackage[utf8]{inputenc}
\usepackage[T1]{fontenc}
\usepackage[ibycus,german,english,swedish]{babel}
\usepackage{xparse}
\usepackage{graphicx}
\usepackage[hyphens]{url}
\usepackage{lettrine}
\usepackage[strict]{csquotes}
\usepackage{setspace}
\usepackage{nomencl}
\usepackage{marginnote}
\usepackage{subfig}
\usepackage{booktabs}
\usepackage[multiple]{footmisc}
\usepackage{verbatim}
\usepackage{ifdraft}
\usepackage{asymptote}
\usepackage{datetime}

\ifdraft{%
  \usepackage{fancyhdr}
  \usepackage{showlabels}
}{%
  \if@twoside
    \usepackage[a4paper,rmargin=6cm]{geometry}
  \fi
}

\usepackage[natbib,style=alphabetic,maxbibnames=99]{biblatex}
\addbibresource{matematik-1.bib}
%\addbibresource{krypto/introcrypt.bib}
%\addbibresource{pwdanalysis/literature.bib}

\usepackage{varioref}
\renewcommand{\reftextbefore}{(föregående sida)}
\renewcommand{\reftextfacebefore}{(föregående sida)}
\renewcommand{\reftextafter}{(nästa sida)}
\renewcommand{\reftextfaceafter}{(nästa sida)}
\renewcommand{\reftextfaraway}[1]{(sidan~\pageref{#1})}
\usepackage{hyperref}
\usepackage[noabbrev,swedish]{cleveref}

\usepackage{amssymb}
\usepackage{amsmath}
\usepackage{amsthm}
\usepackage{mathtools}
\usepackage{mleftright}
\mleftright
\usepackage{thmtools}
% theorems
\declaretheorem[
numberwithin=chapter,
name=Sats,
refname={sats,satserna},
Refname={Sats,Satserna}
]{theorem}
\declaretheorem[
sibling=theorem,
name=Proposition,
refname={proposition,propositionerna},
Refname={Proposition,Propositionerna}
]{proposition}
\declaretheorem[
sibling=theorem,
name=Lemma,
refname={lemma,lemmana},
Refname={Lemma,Lemmana}
]{lemma}
\declaretheorem[
sibling=theorem,
name=Korollarium,
refname={korollarium,korollarierna},
Refname={Korollarium,Korollarierna}
]{corollary}
\declaretheorem[
sibling=theorem,
name=Förmodan,
refname={förmodan,förmodanena},
Refname={Förmodan,Förmodanena},
]{conjecture}
% definitions
\declaretheorem[
numberwithin=chapter,
name=Definition,
refname={definition,definitionerna},
Refname={Definition,Definitionerna},
style=definition
]{definition}
\declaretheorem[
sibling=definition,
name=Axiom,
refname={axiom,axiom},
Refname={Axiom,Axiom},
style=definition
]{axiom}
% remarks
\declaretheorem[
numberwithin=chapter,
name=Anmärkning,
refname={anmärkning,anmärkningarna},
Refname={Anmärkning,Anmärkningarna},
style=remark
]{remark}
% examples, exercises
\declaretheorem[
numberwithin=chapter,
name=Exempel,
refname={exempel,exemplena},
Refname={Exempel,Exemplena},
style=definition
]{example}
\declaretheorem[
numberwithin=chapter,
name=Övning,
refname={övning,övningarna},
Refname={Övning,Övningarna},
style=definition
]{exercise}

\numberwithin{section}{chapter}
\numberwithin{equation}{chapter}
\numberwithin{figure}{chapter}
\numberwithin{table}{chapter}

\renewcommand{\qedsymbol}{Q.E.D.}
\DeclareDocumentCommand{\N}{}{\ensuremath{\mathbb{N}}}
\DeclareDocumentCommand{\Z}{}{\ensuremath{\mathbb{Z}}}
\DeclareDocumentCommand{\Q}{}{\ensuremath{\mathbb{Q}}}
\DeclareDocumentCommand{\R}{}{\ensuremath{\mathbb{R}}}
\DeclareDocumentCommand{\powerset}{}{\ensuremath{\mathcal{P}}}
%\DeclareMathSymbol{\powerset}{\mathord}{MnSyC}{180}
\DeclareDocumentCommand{\U}{}{\ensuremath{\mathcal{U}}}
\DeclareDocumentCommand{\V}{}{\ensuremath{\mathcal{V}}}
\DeclareMathOperator{\card}{card}
\DeclareMathOperator{\tnot}{icke}
\DeclareMathOperator{\tor}{eller}
\DeclareMathOperator{\tand}{och}
\DeclareMathOperator{\congruent}{\equiv}


\makenomenclature{}
\makeindex{}

\includeonly{%
  frontmatter,preface,intro,logik,mangder,game-of-life,%
  naturliga,heltalen,talsystem,backmatter%
}

\begin{document}
\ifdraft{%
  \fancypagestyle{plain}{%
    \renewcommand{\headrulewidth}{0pt}
    \fancyhead{}
    \fancyfoot[co,ce]{\thepage}
    \fancyfoot[ro,le]{%
      \tt Draft: \today~\currenttime{}
    }
  }
  \fancypagestyle{ams}{%
    \renewcommand{\headrulewidth}{0pt}
    \fancyhead[ce]{\small\leftmark}
    \fancyhead[co]{\small\rightmark}
    \fancyhead[lo,re]{}
    \fancyhead[ro,le]{\thepage}
    \fancyfoot[ro,le]{%
      \tt Draft: \today~\currenttime{}
    }
  }
  \pagestyle{ams}
}{}

\title{%
  Matematik för alla, volym I%
}
\author{%
  Daniel Bosk%
}

\frontmatter
\include{frontmatter}
\include{preface}

\mainmatter{}
\ifdraft{%
  \onehalfspacing{}
}{}
% main chapters
\include{intro}
\ifdraft{}{%
  \part{Den matematiska grunden}
}
\include{logik}
\include{mangder}
\include{game-of-life}
\ifdraft{}{%
  \part{Tal och aritmetik}
}
\include{naturliga}
\include{heltalen}
\include{talteori}
\include{rationella}
\include{reella}
\include{talsystem}
\ifdraft{}{%
  \part{Ekvationer}
}
\include{ekvationer}
\include{olikheter}
\include{potensekvationer}
\ifdraft{}{%
  \part{Geometri}
}
\include{geometri}
\include{trigonometri}
\include{linjalg}
\ifdraft{}{%
  \part{Samband och förändring}
}
\include{procent}
\include{forandring}
\ifdraft{}{%
  \part{Kombinatorik, sannolikhet och statistik}
}
\include{kombinatorik}
%\include{pwdanalysis/pwdinclude}
\include{sannolikhet}
\include{statistik}
%\include{krypto/krypto}

% appendices
\appendix
\ifdraft{%
  \doublespacing{}
}{}
\include{kursplanering}

\backmatter{}
\ifdraft{%
  \singlespacing{}
}{}
\printbibliography{}
\listoffigures
\listoftables
%\printnomenclature
\printindex
\include{backmatter}

\clearpage
\end{document}
